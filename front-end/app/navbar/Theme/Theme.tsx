...
const Themes = () => {
  ...
  function swapTheme() {
    const currentTheme = getCookie(THEME_COOKIE_NAME);
    const theme = acceptableTheme.filter(
      (theme: string) => theme != currentTheme
    )[0];
  ...
  useEffect(() => {
    const currentTheme = getCookie(THEME_COOKIE_NAME);
    const theme =
      acceptableTheme.filter((theme: string) => theme == currentTheme)[0] ??
      acceptableTheme[0];
