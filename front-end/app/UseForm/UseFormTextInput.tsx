...
const TextInput = (props: { input: Input; register: any; errors: any }) => {
  const input = props.input;
  const register = props.register;
  const errors = props.errors;
  const validations = { ...input.validations };
  const [isHide, setIsHide] = useState(input.type === "password");
  const errorClassName = "pt-2 px-5  text-error text-sm";
  const svgClassName = "absolute end-5 top-4";
  return (
    <div className="relative w-full ">
      <input
        ...
        className={
          errors[input.name]
            ? "input shadow-md input-bordered w-full  input-secondary"
            : "input  shadow-md input-bordered w-full"
        }
      />
